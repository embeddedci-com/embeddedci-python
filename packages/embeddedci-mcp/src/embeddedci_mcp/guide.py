...
Wiring: the pod has 12 generic LA channels (LA1-LA12) and nothing is fixed, so confirm with the user which DUT signal is on which channel (benchpod://wiring shows an example bench). Target power is eFuse 1 (internal 5 V) or 2 (external). The DUT's reset line goes to the pod's own reset pin: flash(nreset=true), reset_target.

Typical flows:
- Flash and boot: flash(swclk, swdio, target, file, nreset) then power_cycle_and_capture(rx, tx, until_regex).
